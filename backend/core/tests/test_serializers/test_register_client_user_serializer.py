...
class RegisterClientUserSerializerTest(TestCase):
    ...
    def test_validate_fcm_token_blank(self):
        self.invalid_data = self.valid_data.copy()
        self.invalid_data['fcm_token'] = ""
        serializer = RegisterClientUserSerializer(data=self.invalid_data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("fcm_token", serializer.errors)
        self.assertEqual(
            str(serializer.errors["fcm_token"][0]),
            "fcm token is invalid."
        )

    def test_validate_fcm_token_null(self):
        self.invalid_data = self.valid_data.copy()
        self.invalid_data['fcm_token'] = None
        serializer = RegisterClientUserSerializer(data=self.invalid_data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("fcm_token", serializer.errors)
        self.assertEqual(
    ...
    def test_validate_fcm_token_invalid(self):
        self.invalid_data = self.valid_data.copy()
        self.invalid_data['fcm_token'] = []
        serializer = RegisterClientUserSerializer(data=self.invalid_data)
        self.assertFalse(serializer.is_valid())
    ...
    def test_validate_fcm_token_required(self):
        self.invalid_data = self.valid_data.copy()
        del self.invalid_data['fcm_token']
        serializer = RegisterClientUserSerializer(data=self.invalid_data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("fcm_token", serializer.errors)
        self.assertEqual(
